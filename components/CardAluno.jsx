    ...
    <div className="glass-panel flex flex-col justify-between overflow-hidden p-6 transition hover:-translate-y-1 hover:shadow-glow">
      <div className="flex items-center gap-4">
        <div className="relative h-16 w-16 overflow-hidden rounded-2xl border border-outline/60">
          <Image
            src={aluno?.profiles?.avatar_url ?? '/avatars/default.svg'}
            alt={aluno?.profiles?.full_name ?? 'Aluno'}
            fill
        ...
        <div>
          ...
          <h3 className="text-2xl font-semibold text-white">{aluno?.profiles?.full_name}</h3>
      ...
      <div className="mt-6 space-y-4">
        <div className="flex items-center justify-between text-sm text-slate-300">
          ...
          <span>{progresso.semana} / {progresso.meta}</span>
        </div>
        ...
        <div className="flex items-center justify-between text-xs text-slate-400">
          <span className="flex items-center gap-2"><TrendingUp size={14} /> Performance semanal</span>
          <span className="flex items-center gap-1 text-neon"><Trophy size={14} /> {percentual}%</span>
        </div>
      </div>
