...
const CardExercicio = ({ exercicio, prescription }) => {
  if (!exercicio) return null;
  return (
    <div className="glass-panel flex flex-col overflow-hidden">
      <div className="relative h-40 w-full overflow-hidden">
        <Image src={exercicio.imagem} alt={exercicio.nome} fill sizes="(min-width: 1024px) 33vw, 100vw" className="object-cover" />
      ...
      <div className="flex flex-1 flex-col gap-4 px-6 py-5">
        <div>
          ...
          <p className="text-sm text-slate-400">{exercicio.foco}</p>
        </div>
        {prescription ? (
          <div className="grid grid-cols-3 gap-3 text-center text-xs font-medium text-slate-300">
            ...
            <div className="rounded-2xl border border-outline/60 bg-surface/40 px-3 py-2">
              ...
              <span>{prescription.intensidade ?? 'Zona 3'}</span>
